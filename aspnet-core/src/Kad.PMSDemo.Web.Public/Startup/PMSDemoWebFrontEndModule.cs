...
namespace Kad.PMSDemo.Web.Public.Startup
{
    [DependsOn(
        typeof(PMSDemoWebCoreModule)
    )]
    public class PMSDemoWebFrontEndModule : AbpModule
    {
        private readonly IConfigurationRoot _appConfiguration;

        public PMSDemoWebFrontEndModule(IWebHostEnvironment env, PMSDemoEntityFrameworkCoreModule abpZeroTemplateEntityFrameworkCoreModule)
        {
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(PMSDemoWebFrontEndModule).GetAssembly());
        }
    }
}
